...
prepFigTable <- function(ctt,variant_) {
  ...
  ctreg=lm(new_genvalue~cohort+as.factor(diffvariantbin)+age_category+labname+gender ,data=ctt)
  
  
  #intercepts
  facs_=c()
  for(j_ in 1:vbins){facs_=c(facs_,paste0('as.factor(diffvariantbin)',j_))}
  ...
  intercept=as.numeric(ctreg$coefficients[1])+intercept_variant_age+intercept_variant_bin+intercept_variant_gender
  vc=vcov(ctreg)
